...
If you prefer a simpler installation process, this project also supports integration with the [Nexus](https://github.com/zerthox/nexus) addon framework. This method provides a user-friendly interface and automatic management.

## Nexus Setup Steps

### Step 1: Install Nexus
First, download and install Nexus from the [official repository](https://github.com/zerthox/nexus). Follow their installation instructions for your platform.

### Step 2: Build with Nexus Support
Instead of using the pre-built release, you'll need to compile the project with Nexus features enabled:

```bash
# Clone the repository
git clone https://github.com/SorryQuick/external-dx11-overlay.git
cd external-dx11-overlay

# Build with Nexus support
cargo build --features nexus --release
```

### Step 3: Install the Addon
1. Copy the compiled DLL (`target/release/external_dx11_overlay.dll`) to your Nexus addons directory
2. Launch Guild Wars 2 with Nexus enabled

### Step 4: Configure BlishHUD
1. Once in-game, enable the "Blish HUD overlay loader" addon in Nexus
2. Press `ALT+SHIFT+1` to open the configuration window
3. Click "Browse for Executable..." and select your BlishHUD executable file
4. Check "Launch on Startup" if you want BlishHUD to start automatically
...
- **Better Error Handling**: Nexus provides better feedback and error reporting
- **Automatic Updates**: Easier to update when new versions are available
...
| **Startup** | Automatic via Nexus | Run separate loader executable |
| **User Experience** | More user-friendly | More technical setup required |
| **Error Diagnosis** | Better error reporting | Manual log checking required |

## Switching from Traditional to Nexus

If you're already using the traditional method and want to switch to Nexus:

1. Build the project with Nexus features as shown above
2. Install Nexus if you haven't already
3. Copy the Nexus-enabled DLL to your Nexus addons directory
