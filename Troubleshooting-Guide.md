...
1. **Nexus framework itself**
2. **The external-dx11-overlay dll (Nexus version)**
3. **The BlishHUD executable**
4. **Nexus addon configuration**

### Is Nexus Loading Properly?

- **Check if Nexus is running**: Look for Nexus processes in your task manager
- **Verify addon is enabled**: In-game, check that "Blish HUD overlay loader" is enabled in Nexus
- **Check Nexus logs**: Nexus usually provides its own logging system - check their documentation for log locations
...
- **Keybind conflict**: ALT+SHIFT+1 might be bound to something else in GW2 or another addon
- **Addon not loaded**: Verify the DLL was placed in the correct Nexus addons directory
- **Nexus version compatibility**: Ensure you're using a compatible version of Nexus

#### BlishHUD Won't Launch from Nexus Interface
- **Incorrect executable path**: Double-check the path in the configuration window
- **Permissions issue**: Ensure the BlishHUD executable has proper permissions
- **Missing dependencies**: Verify dotnet48 or other requirements are met in your environment

#### Nexus Integration Not Available
- **Build issue**: Make sure you built with `cargo build --features nexus --release`
...
1. Build without the nexus feature: `cargo build --release`
2. Use the traditional setup process from the Simple User Guide
3. This can help determine if the issue is with your BlishHUD setup or the Nexus integration specifically

### Nexus Error Messages

Common Nexus-related error messages and solutions:

- **"Addon failed to load"**: Check that the DLL is not corrupted and is in the right location
- **"Missing dependency"**: Ensure all required DLLs are present
- **"Keybind registration failed"**: Another addon might have conflicting keybinds
- **"Texture loading failed"**: Check that Nexus texture loading is working with other addons
...
- Ensure you're using the correct DLL built with `cargo build --features nexus --release`, not the standard build
- Verify Nexus is properly installed and running before launching GW2
- Check that the addon appears in Nexus's addon list (if it doesn't appear, the DLL might be in the wrong location or corrupted)
- If the configuration window doesn't open with ALT+SHIFT+1, check for keybind conflicts with GW2 or other addons
